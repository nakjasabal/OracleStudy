...
시나리오] 앞에서 생성한 뷰를 다음과 같이 수정하시오.
    기존 컬럼인 employee_id, first_name, job_id, hire_date, 
    department_id를
...
--문제의 조건대로 select문 작성
select
    employee_id, concat(first_name||' ', last_name), email,
    manager_id
...
select
  ...
  trim(to_char((salary+(salary*nvl(commission_pct,0)))
        *12,'999,000'))
from employees;
--뷰로 생성
/* 생성시 연산식이 추가되어 논리적으로 컬럼이 추가되는 경우에는 반드시
별칭으로 컬럼명을 명시해야한다. 그렇지 않으면 에러가 발생한다. */
...
create or replace view view_test1 (jid, jcnt, javg)
as 
select job_id, count(*), round(avg(salary)) from employees group by job_id;
